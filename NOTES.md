...
1. 紹介ページ（GitHub Pages）に**ダウンロードのボタン**を足した。リンクは
   `releases/latest/download/...` の固定 URL なので、版を上げても古い版を指さない
   （QR も `releases/latest` へ。前は `tag/v0.17.0` 決め打ちで v0.18.0 に置いていかれていた）
2. 「最新版 vX.Y.Z」の表示は pages ワークフローがデプロイのたびに `gh release view` で
   差し替える（HTML 側は `<b id="ver">` が目印。中の v0.18.0 は控えの値）。リリース後は release.yml が
   pages.yml を呼んで出し直す
3. **人の目で確かめる**のは次のタグを切った後 — Actions の release → pages が続けて走り、
   ページの版表示が新しいタグになるか
