...
ファイル名やゲームの語ではなく型で判じるための問い — セル目を持つ文書だけが
盤面エディタの担当になる。
-}
...
{-| オブジェクトを書いた順のまま対にする。"//" で始まるキーはアノテーションとして読み飛ばす。
D.keyValuePairs に直接デコーダを渡すとアノテーションの値までデコードして失敗するので、
